resource "azurerm_vpn_server_configuration" "vpn_server_config" {
  ...
  azure_active_directory_authentication {
    audience = azuread_service_principal.azure_vpn[0].application_id
...
resource "azuread_service_principal_delegated_permission_grant" "azure_vpn" {
  ...
  service_principal_object_id          = azuread_service_principal.azure_vpn[0].object_id
  resource_service_principal_object_id = data.azuread_service_principal.windows_azure_active_directory[0].object_id
  ...
  conditions {
    applications {
      included_applications = [
        azuread_service_principal.azure_vpn[0].application_id
      ]
    }
